...
export async function checkUpdate(): Promise<IAppVersion | undefined> {
  ...
  const res = await axios.get(
    'https://github.com/Cail-Gainey/mihomo-party-checkin/releases/latest/download/latest.yml',
    {
...
export async function downloadAndInstallUpdate(version: string): Promise<void> {
  ...
  const fileMap = {
    'win32-x64': `mihomo-party-checkin-windows-${version}-x64-setup.exe`,
    'win32-ia32': `mihomo-party-checkin-windows-${version}-ia32-setup.exe`,
    'win32-arm64': `mihomo-party-checkin-windows-${version}-arm64-setup.exe`,
    'darwin-x64': `mihomo-party-checkin-macos-${version}-x64.pkg`,
    'darwin-arm64': `mihomo-party-checkin-macos-${version}-arm64.pkg`
  }
  ...
  try {
    ...
    if (file.endsWith('.pkg')) {
      try {
        ...
        const shell = `installer -pkg ${path.join(dataDir(), file).replace(/ /g, '\\\\ ')} -target /`
