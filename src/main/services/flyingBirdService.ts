/**
 * FlyingBird服务模块
 * 处理与FlyingBird网站相关的操作，包括签到和获取订阅
...
/**
 * FlyingBird 签到服务
 * 登录FlyingBird网站并执行签到操作
 * @param email 用户邮箱
 * @param password 用户密码
 * @param url FlyingBird网站地址
 * @returns 签到结果
 * @author Cail Gainey <cailgainey@foxmail.com>
 */
export async function checkin(email: string, password: string, url: string) {
  try {
    ...
    console.log(`[${email}] 正在登录FlyingBird...`)
    ...
    console.error('FlyingBird 签到出错:', error)
    return {
      success: false,
...
/**
 * 获取 FlyingBird 订阅链接
 * 登录 FlyingBird 并从用户页面获取 Clash 订阅链接
 * @param email 用户邮箱
 * @param password 用户密码
 * @param url FlyingBird网站地址
 * @returns 订阅链接获取结果
...
export async function getSubscription(email: string, password: string, url: string) {
  try {
    ...
    console.log(`[${email}] 正在登录FlyingBird以获取订阅...`)
    ...
    // 根据提供的按钮代码，FlyingBird的链接格式为：https://fbapiv3.fbsublink.com/flydsubal/spd9osf5lqu8o7wg?clash=1&amp;extend=1
    const clashLinkRegex = /data-clipboard-text="(https:\/\/fbapiv3\.fbsublink\.com\/[^"]+\?clash=\d+[^"]+)"/i
    const match = htmlContent.match(clashLinkRegex)
    
    if (match && match[1]) {
      // 替换HTML实体 &amp; 为 &
      const subscriptionUrl = match[1].replace(/&amp;/g, '&')
      return { 
        success: true, 
        subscriptionUrl
      }
    ...
    console.error('获取 FlyingBird 订阅链接出错:', error)
    return {
      success: false,
