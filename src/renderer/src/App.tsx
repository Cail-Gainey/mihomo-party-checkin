...
import CheckinCard from '@renderer/components/sider/checkin-card'
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setOrder(siderOrder || defaultOrder)
    setSiderWidthValue(siderWidth)
  }, [])

  useEffect(() => {
    if (appConfig && appConfig.siderOrder) {
      setOrder(appConfig.siderOrder)
    }
    if (appConfig && appConfig.siderWidth) {
      setSiderWidthValue(appConfig.siderWidth)
    }
  }, [appConfig])
            ...
            <div className="h-full w-full flex flex-col gap-2">
              {order.length > 0 ? (
                ...
                Object.entries(componentMap).map(([key, Component]) => (
                  <Component key={key} iconOnly={true} />
                ))
              )}
            </div>
          </div>
